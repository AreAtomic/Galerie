...
public class GalerieController {
     ...
     *
     * @param galerie initialisé par Spring, valeurs par défaut à afficher dans le
     *                formulaire
     ...
     *
     * @param galerie      Une galerie initialisée avec les valeurs saisies dans le
     *                     formulaire
    ...
    public String ajouteLaGaleriePuisMontreLaListe(Galerie galerie, RedirectAttributes redirectInfo) {
        ...
        try {
            ...
            // En cas de doublon, JPA lève une exception de violation de contrainte
            // d'intégrité
     ...
     *
     * @param galerie      à partir de l'id de la galerie transmis en paramètre,
     *                     Spring fera une requête SQL SELECT pour chercher la
     *                     galerie dans la base
     * @param redirectInfo pour transmettre des paramètres lors de la redirection
     * @return une redirection vers l'affichage de la liste des galeries
     */
    @GetMapping(path = "delete")
    public String supprimeUneCategoriePuisMontreLaListe(@RequestParam("id") Galerie galerie,
            RedirectAttributes redirectInfo) {
        String message = "La galerie '" + galerie.getNom() + "' a bien été supprimée";
        try {
            dao.delete(galerie); // Ici on peut avoir une erreur (Si il y a des expositions pour cette galerie
                                 // par exemple)
        } catch (DataIntegrityViolationException e) {
            // violation de contrainte d'intégrité si on essaie de supprimer une galerie qui
            // a des expositions
            message = "Erreur : Impossible de supprimer la galerie '" + galerie.getNom()
                    + "', il faut d'abord supprimer ses expositions";
        }
        // RedirectAttributes permet de transmettre des informations lors d'une
        // redirection,
        // Ici on transmet un message de succès ou d'erreur
